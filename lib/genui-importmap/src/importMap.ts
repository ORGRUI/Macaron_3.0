export type RendererImportMap = { imports?: Record<string, string>; scopes?: Record<string, Record<string, string>> };
...
const collectBareModuleSpecifiers = (code: string, specifiers: Set<string>, start = 0, end = code.length) => {
  for (let index = start; index < end; index += 1) {
    ...
    if (char === "`") {
      index = skipTemplateLiteral(code, index, (expressionStart, expressionEnd) => collectBareModuleSpecifiers(code, specifiers, expressionStart, expressionEnd));
      continue;
    }
    if (isKeywordAt(code, index, "import")) {
      ...
      if (code[afterImport] === "(") {
        ...
        if (literal?.value && isBareModuleSpecifier(literal.value)) specifiers.add(literal.value);
      ...
      if (from?.specifier && isBareModuleSpecifier(from.specifier)) specifiers.add(from.specifier);
...
export function extractBareModuleSpecifiers(code: string) {
  ...
  collectBareModuleSpecifiers(code, specifiers);
  return specifiers;
}
...
export function prepareRendererImportMap(map: RendererImportMap | null | undefined, baseUrl = typeof location === "undefined" ? undefined : location.href) {
  ...
  if (!map?.scopes) return { imports };
  const scopes = Object.fromEntries(Object.entries(map.scopes).map(([scope, entries]) => [scope, resolveEntries(entries)]));
...
export function urlImportMap(source: string, options?: { kind?: ImportMapProvider["kind"]; fetch?: (source: string) => Promise<Pick<Response, "ok" | "status" | "statusText" | "json">> }): ImportMapProvider {
  ...
  const load = () =>
    (loadPromise ??= (async () => {
      ...
      return (await response.json()) as RendererImportMap;
    })());
...
export function packageImportMap(packageName: string, options?: { resolve?: (name: string) => string }): ImportMapProvider {
  return {
    resolve: async () => {
      ...
      return { imports: Object.fromEntries(Object.entries(importMap.imports ?? {}).map(([specifier, target]) => [specifier, rebase(target)])) };
    },
  };
}
